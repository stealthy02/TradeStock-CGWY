...
// 饼状图接口请求参数
const getPieQueryParams = (type: 'all' | 'year' | 'month') => {
  return {
    time_type: type
  };
};
...
const formatStatisticCard = (data: HomeStatisticCard) => {
  statisticCard.value = {
    ...
    last_month_profit: { title: '上月毛利', value: data.last_month_profit || 0 },
...
const fetchTrendChartData = async () => {
  try {
    ...
    const trendRes = await getTrendChart(queryParams);
...
const fetchPieChartData = async () => {
  try {
    const queryParams = getPieQueryParams(pieTimeType.value);
    const pieRes = await getPieChart(queryParams);
    formatPieChart(pieRes.data);
  } catch (err) {
    purchaserProfitData.value = [];
    productProfitData.value = [];
    console.error('获取饼状图数据失败：', err);
  }
};
