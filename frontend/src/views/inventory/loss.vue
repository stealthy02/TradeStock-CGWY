<template>
  <div class="page-container">
    ...
    >
      ...
      <a-form-item>
        <a-button type="primary" @click="() => fetchData(undefined, true)">查询</a-button>
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'product_name'">
          <template v-if="record.key === 'new-row'">
            <a-select
              v-model:value="newRowProductSelectValue"
              placeholder="请选择商品"
              show-search
              :filter-option="false"
              ...
              @change="(val: any, option: any) => handleNewRowProductChange(val, option)"
              style="width: 100%"
            />
          ...
          <template v-else-if="record.isEditing">
            <a-select
              v-model:value="editingRowProductSelectValue[record.id]"
              placeholder="请选择商品"
              show-search
              :filter-option="false"
              :options="productOptions"
              @search="handleProductSearch"
              @change="(val: any, option: any) => handleEditingRowProductChange(record.id, val, option)"
              style="width: 100%"
            />
        ...
        <template v-else-if="column.key === 'product_spec'">
          ...
          <template v-else-if="record.isEditing">
            <a-input
              ...
              placeholder="请输入商品规格，例如：30"
              @blur="() => handleEditingRowSpecBlur(record.id)"
              style="width: 100%"
            />
          </template>
...
const newRowProductSelectValue = ref<string>('');
const editingRowProductSelectValue = ref<Record<number, string>>({});

const route = useRoute();

const productOptions = ref<{ label: string; value: string; productName: string; spec: string; inventory: any }[]>([]);
...
const fetchData = async (id?: number, resetPage: boolean = false) => {
  if (resetPage) {
...
const handleProductSearch = async (value: string) => {
  if (!value) return;
  try {
    const response = await getInventoryList({ product_name: value, page_size: 10 });
    if (Array.isArray(response.data.list)) {
      // 用于新增/编辑行的选项（包含规格信息）
      productOptions.value = response.data.list.map((item: InventoryItem) => ({
        label: `${item.product_name}(${item.product_spec})`,
        value: `${item.product_name}|${item.product_spec}`,
        productName: item.product_name,
        spec: item.product_spec,
        inventory: item
      }));
      // 用于搜索的选项（去重，只显示商品名称）
      const uniqueNames = [...new Set(response.data.list.map((item: InventoryItem) => item.product_name))];
      searchProductOptions.value = uniqueNames.map(name => ({
        label: name,
        value: name
      }));
    } else {
...
const handleProductFocus = async () => {
  try {
    const response = await getInventoryList({ page_size: 10 });
    if (Array.isArray(response.data.list)) {
      // 用于新增/编辑行的选项（包含规格信息）
      productOptions.value = response.data.list.map((item: InventoryItem) => ({
        label: `${item.product_name}(${item.product_spec})`,
...
const getInventoryByProductNameAndSpec = async (productName: string, productSpec: string): Promise<InventoryItem | null> => {
  try {
    const response = await getInventoryList({ product_name: productName, product_spec: productSpec, page_size: 1 });
...
const handleNewRowProductChange = async (value: SelectValue, option: any) => {
  if (!value) {
  ...
  // 从选项中获取商品名称和规格
  const productName = option?.productName || option?.inventory?.product_name;
  const productSpec = option?.spec || option?.inventory?.product_spec;
  if (!productName || !productSpec) {
    newRowCurrentInventory.value = null;
    newRowInventoryError.value = '请选择带规格的商品';
    return;
  }
  
  newRow.product_name = productName;
  newRow.product_spec = productSpec;
  
  const inventory = await getInventoryByProductNameAndSpec(productName, productSpec);
  if (inventory) {
    newRowCurrentInventory.value = inventory;
    newRowInventoryError.value = '';
    
    if (newRow.loss_num > inventory.inventory_num) {
      newRowInventoryError.value = `库存不足，当前${productName}(${productSpec})库存为${inventory.inventory_num}`;
    }
  } else {
    newRowCurrentInventory.value = null;
...
const handleEditingRowProductChange = async (id: number, value: SelectValue, option: any) => {
  if (!value) {
  ...
  if (!productName || !productSpec) {
    editingRowCurrentInventory.value[id] = null;
    editingRowInventoryError.value[id] = '请选择带规格的商品';
    return;
  }
  ...
  if (inventory) {
    ...
    if (editingRows.value[id]) {
      editingRows.value[id].product_name = productName;
      editingRows.value[id].product_spec = productSpec;
      
      if (editingRows.value[id].loss_num > inventory.inventory_num) {
        editingRowInventoryError.value[id] = `库存不足，当前${productName}(${productSpec})库存为${inventory.inventory_num}`;
      }
    }
  } else {
    editingRowCurrentInventory.value[id] = null;
...
// 处理新增行规格输入框失焦事件
const handleNewRowSpecBlur = async () => {
  if (!newRow.product_name || !newRow.product_spec) {
    return;
  }
  
  const inventory = await getInventoryByProductNameAndSpec(newRow.product_name, newRow.product_spec);
  if (inventory) {
    newRowCurrentInventory.value = inventory;
    newRowInventoryError.value = '';
    
    if (newRow.loss_num > inventory.inventory_num) {
      newRowInventoryError.value = `库存不足，当前${newRow.product_name}(${newRow.product_spec})库存为${inventory.inventory_num}`;
    }
  } else {
    newRowCurrentInventory.value = null;
    newRowInventoryError.value = '商品不存在';
...
// 处理编辑行规格输入框失焦事件
const handleEditingRowSpecBlur = async (id: number) => {
  if (!editingRows.value[id]?.product_name || !editingRows.value[id]?.product_spec) {
    return;
  }
  
  const productName = editingRows.value[id].product_name;
  const productSpec = editingRows.value[id].product_spec;
...
const handleAdd = async () => {
  ...
  try {
    ...
    newRowCurrentInventory.value = null;
    newRowInventoryError.value = '';
    newRowProductSelectValue.value = '';
...
const handleCancelEdit = (id: number) => {
  ...
  delete editingRowProductSelectValue.value[id];
};
