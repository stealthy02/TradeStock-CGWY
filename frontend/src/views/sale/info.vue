<template>
  <div class="page-container">
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'sale_price'">
          ...
          <template v-else>{{ record.sale_price ?? '-' }}</template>
        </template>
        <template v-else-if="column.key === 'total_price'">
          ...
          <template v-else>{{ record.total_price ?? '-' }}</template>
        </template>
        <template v-else-if="column.key === 'total_profit'">
          ...
          <template v-else>{{ record.total_profit ?? '-' }}</template>
        </template>
    ...
    <div class="pagination-container">
      <a-pagination
        v-model:current="searchParams.page_num"
        v-model:pageSize="searchParams.page_size"
        :total="total"
...
import type { AddSaleInfoReq, UpdateSaleInfoReq, SaleInfoListQuery } from '@/types';
import {
  addSaleInfo,
  getSaleInfoList,
  updateSaleInfo,
...
const fetchData = async (id?: number | MouseEvent, resetPage: boolean = false) => {
  if (resetPage) {
...
const handleEditingRowProductSearch = async (id: number, value: string) => {
  if (editingRows.value[id]) {
    editingRows.value[id].product_name = value;
  }
  try {
    const response = await getSaleProductSelect({ keyword: value });
    let options: { label: string; value: string }[] = [];
    ...
    if (value) {
      ...
      editingRowProductOptions.value[id] = options;
    }
  } catch (error) {
    console.error('获取商品列表失败:', error);
    if (value) {
      editingRowProductOptions.value[id] = [{ label: value, value: value }];
    } else {
      editingRowProductOptions.value[id] = [];
    }
  }
