...
class SaleInfoRepository:
    ...
    def list_by_statement(self, purchaser_id: int, statement_id: int, start_date: Optional[date] = None, end_date: Optional[date] = None) -> List[Dict]:
    ...
    def update_statement_id_for_sales(self, statement_id: int, new_statement_id: int, start_date: date) -> None:
        """
        更新销售记录的对账单ID
        """
        self.db.query(SaleInfo).filter(
    ...
    # ==================== 基于原始日期(sale_date)的统计方法 ====================
    
    def get_total_revenue_by_date(self, start_date: date, end_date: date) -> Decimal:
        """
        获取指定日期范围内的销售总收入（基于sale_date）
        """
        result = self.db.query(func.sum(SaleInfo.sale_total_price)).filter(
            SaleInfo.is_deleted == False,
            SaleInfo.sale_date >= start_date,
            SaleInfo.sale_date <= end_date
        ).scalar()
        return Decimal(str(result)) if result is not None else Decimal("0.00")
    
    def get_total_profit_by_date(self, start_date: date, end_date: date) -> Decimal:
        """
        获取指定日期范围内的销售总利润（基于sale_date）
        """
        result = self.db.query(func.sum(SaleInfo.total_profit)).filter(
            SaleInfo.is_deleted == False,
            SaleInfo.sale_date >= start_date,
            SaleInfo.sale_date <= end_date
        ).scalar()
    ...
    def get_purchaser_profit_distribution(self, start_date: date, end_date: date) -> List[Dict]:
        """
        获取指定日期范围内的采购商利润分布（基于sale_date）
        """
        results = self.db.query(
            Purchaser.purchaser_name,
            func.sum(SaleInfo.total_profit).label("profit")
        ).join(
            SaleInfo, SaleInfo.purchaser_id == Purchaser.id
        ).filter(
            SaleInfo.is_deleted == False,
            SaleInfo.sale_date >= start_date,
            SaleInfo.sale_date <= end_date
    ...
    def get_product_profit_distribution(self, start_date: date, end_date: date) -> List[Dict]:
        """
        获取指定日期范围内的商品利润分布（基于sale_date）
        """
        results = self.db.query(
            Goods.goods_name,
            func.sum(SaleInfo.total_profit).label("profit")
        ).join(
            SaleInfo, SaleInfo.goods_id == Goods.id
        ).filter(
            SaleInfo.is_deleted == False,
            SaleInfo.sale_date >= start_date,
            SaleInfo.sale_date <= end_date
