...
class GoodsRepository:
    ...
    def get_by_name_and_spec(self, name: str, spec: int, include_deleted: bool = False) -> Optional[Dict]:
        """
        根据名称和规格获取商品信息
        ...
        Args:
            name (str): 商品名称
            spec (int): 商品规格
            include_deleted (bool): 是否包含软删除的商品，默认为False
        
        Returns:
            Optional[Dict]: 商品信息字典，不存在返回None
        """
        query = self.db.query(Goods).filter(
            Goods.goods_name == name,
            Goods.product_spec == spec
        )
        if not include_deleted:
            query = query.filter(Goods.is_deleted == False)
        obj = query.first()
        return self._to_dict(obj) if obj else None
    
    def restore_if_deleted(self, name: str, spec: int) -> Optional[int]:
        """
        如果商品被软删除则恢复它
        ...
        Returns:
            Optional[int]: 恢复的商品ID，如果不存在则返回None
        """
        obj = self.db.query(Goods).filter(
            Goods.goods_name == name,
            Goods.product_spec == spec,
    ...
    def count_by_inventory_conditions(self, name: Optional[str], spec: Optional[int],
        ...
        if spec is not None:
            query = query.filter(Goods.product_spec == spec)
    ...
    def list_by_inventory_conditions(self, name: Optional[str], spec: Optional[int], min_num: Optional[int],
    ...
    def get_last_purchase_date(self, goods_id: int) -> Optional[date]:
        """
        获取商品最后采购日期
    ...
    def get_last_sale_date(self, goods_id: int) -> Optional[date]:
        """
        获取商品最后销售日期
