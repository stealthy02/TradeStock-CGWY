...
class SaleStatementRepository:
    ...
    def get_total_statement_amount_by_date(self, start_date: date, end_date: date) -> Decimal:
        """
        获取指定时间范围内的销售对账单总金额
        ...
        result = self.db.query(func.sum(SaleStatement.statement_amount)).filter(
            ...
            or_(
                ...
                (SaleStatement.end_date >= start_date) & 
                (SaleStatement.end_date <= end_date),
    ...
    def get_monthly_revenue_expend(self, start_date: date, end_date: date) -> List[Dict]:
        """
        获取指定时间范围内的月度营收支出数据
        说明：end_date为空的记录视为本日
        """
        ...
        purchase_results = self.db.query(
            ...
            or_(
                ...
                (PurchaseStatement.end_date >= start_date) & 
                (PurchaseStatement.end_date <= end_date),
                # end_date为空且视为今天，今天在时间范围内
                (PurchaseStatement.end_date == None) & 
                (today_date >= start_date) & 
                (today_date <= end_date)
