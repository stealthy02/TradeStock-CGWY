...
class PurchaseInfoRepository:
    ...
    def list_by_statement(self, supplier_id: int, statement_id: int, start_date: Optional[date] = None, end_date: Optional[date] = None) -> List[Dict]:
    ...
    def update_statement_id_for_purchases(self, statement_id: int, new_statement_id: int, start_date: date) -> None:
        """
        更新采购记录的对账单ID
        """
        self.db.query(PurchaseInfo).filter(
    ...
    # ==================== 基于原始日期(purchase_date)的统计方法 ====================
    
    def get_total_expend_by_date(self, start_date: date, end_date: date) -> Decimal:
        """
        获取指定日期范围内的采购总支出（基于purchase_date）
        """
        result = self.db.query(func.sum(PurchaseInfo.purchase_total_price)).filter(
            PurchaseInfo.is_deleted == False,
            PurchaseInfo.purchase_date >= start_date,
            PurchaseInfo.purchase_date <= end_date
        ).scalar()
