...
class InventoryLossRepository:
    ...
    # ==================== 基于原始日期(loss_date)的统计方法 ====================
    
    def get_total_loss_cost_by_date(self, start_date: date, end_date: date) -> Decimal:
        """
        获取指定日期范围内的报损总成本（基于loss_date）
        """
        result = self.db.query(func.sum(InventoryLoss.loss_total_cost)).filter(
            InventoryLoss.is_deleted == False,
            InventoryLoss.loss_date >= start_date,
            InventoryLoss.loss_date <= end_date
        ).scalar()
