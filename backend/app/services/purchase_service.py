...
async def add_purchase(data) -> Dict[str, Any]:
    ...
    try:
        ...
        inventory_flow_repo.create({
            ...
            "stock_unit_cost": new_cost,  # 记录当时的单位成本
        ...
        await recalculate_from_date(goods_id, purchase_date)
    ...
    try:
        ...
        for item in list_data:
            ...
            formatted_list.append({
                ...
                "purchase_num": format_number(item["purchase_num"]),
...
async def update_purchase(data) -> None:
    ...
    try:
        ...
        old_num = float(old_record["purchase_num"])
        ...
        # 获取新供货商ID（如果有变更）
        new_supplier_id_for_check = old_supplier_id
        if hasattr(data, "supplier_name"):
            supplier = supplier_repo.get_by_name(data.supplier_name)
            if supplier:
                new_supplier_id_for_check = supplier["id"]
        
        # 解析新日期
        new_date = datetime.strptime(data.purchase_date, "%Y-%m-%d").date()
        
        # 检查原日期在原供货商当前对账单中是否合法
        old_current_statement = statement_repo.get_by_supplier(old_supplier_id)
        if old_current_statement:
            ...
            if old_start_date:
                ...
                if old_purchase_date_to_check < old_start_date:
                    raise CustomAPIException(code=604, message="该采购记录早于当前对账单开始日期，禁止修改")
        
        # 检查新日期在新供货商当前对账单中是否合法
        new_current_statement = statement_repo.get_by_supplier(new_supplier_id_for_check)
        if new_current_statement:
            ...
            if new_start_date:
                ...
                if new_date < new_start_date:
                    raise CustomAPIException(code=605, message="新采购日期早于当前对账单开始日期，禁止修改")
        ...
        if new_product_name and new_product_name != goods["goods_name"]:
            # 先尝试恢复被软删除的商品
            restored_id = goods_repo.restore_if_deleted(new_product_name, new_product_spec)
            if restored_id:
                # 商品被恢复，重置库存和成本
                new_goods_id = restored_id
                new_current_stock = 0
        ...
        inventory_flow_repo.create({
            ...
            "stock_unit_cost": final_cost,  # 记录当时的单位成本
            "oper_time": new_date,
        ...
        # 确保日期类型一致
        if isinstance(old_purchase_date, str):
            old_purchase_date = datetime.strptime(old_purchase_date, "%Y-%m-%d").date()
        # 取较早的日期
        recalc_date = old_purchase_date if old_purchase_date and old_purchase_date < new_date else new_date
        # 如果商品变更了，需要重算旧商品和新商品
...
async def get_purchase_bill_detail(bill_id: int,  end_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    for p in purchase_list:
        ...
        if key not in merged_purchases:
            ...
            purchase_num = format_number(p["purchase_num"])
...
async def export_purchase_bill(bill_id: int, end_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "filename": f"采购对账单_{data['bill_info']['supplier_name']}_{datetime.now().date().strftime('%Y%m%d')}.xlsx"
    }