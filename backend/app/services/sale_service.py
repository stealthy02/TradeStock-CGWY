...
def format_number(value: Union[int, float]) -> Union[int, float]:
    """格式化数字，如果是整数则返回int类型，否则返回float类型"""
    if value is None:
        return 0
...
async def add_sale(data) -> Dict[str, Any]:
    ...
    print(f'''销售信息录入：采购商={purchaser_name}, 商品={product_name}, 规格={product_spec}, 客户商品名={customer_product_name}, 销售数量={sale_num}, 销售单价={unit_price}, 总金额={total_price}, 销售日期={data.sale_date}''')
    if unit_price <= 0:
    ...
    # 2. 获取或创建商品（按名称和规格组合）
    # 先尝试恢复被软删除的商品
    restored_id = repo.goods.restore_if_deleted(product_name, product_spec)
    if restored_id:
        # 商品被恢复，重置库存和成本
        goods_id = restored_id
        current_stock = 0
        unit_cost = 0.00
        # 重置商品库存和成本
        ...
        if not goods:
            ...
            goods_id = goods["id"]
            current_stock = float(goods["current_stock_num"])
    ...
    # 9. 触发成本重算
    from app.services.cost_recalc_service import recalculate_from_date
    await recalculate_from_date(goods_id, sale_date)
...
async def list_sale_info(
    ...
    for item in list_data:
        formatted_list.append({
            ...
            "sale_num": format_number(item["sale_num"]),
...
async def update_sale(data) -> None:
    ...
    # 获取新采购商ID（如果有变更）
    new_purchaser_id_for_check = old_purchaser_id
    if hasattr(data, "purchaser_name"):
        purchaser = repo.purchaser.get_by_name(data.purchaser_name)
        if purchaser:
            new_purchaser_id_for_check = purchaser["id"]
    
    # 解析新日期
    new_date = datetime.strptime(data.sale_date, "%Y-%m-%d").date()
    
    # 检查原日期在原采购商当前对账单中是否合法
    old_current_statement = repo.sale_statement.get_by_purchaser(old_purchaser_id)
    if old_current_statement:
        old_start_date = old_current_statement.get("start_date")
        if old_start_date:
            if hasattr(old_start_date, "date"):
                old_start_date = old_start_date.date()
            if hasattr(old_sale_date, "date"):
                old_sale_date_to_check = old_sale_date.date()
            else:
                old_sale_date_to_check = old_sale_date
            if old_sale_date_to_check < old_start_date:
                raise CustomAPIException(code=604, message="该销售记录早于当前对账单开始日期，禁止修改")
    
    # 检查新日期在新采购商当前对账单中是否合法
    new_current_statement = repo.sale_statement.get_by_purchaser(new_purchaser_id_for_check)
    if new_current_statement:
        new_start_date = new_current_statement.get("start_date")
        if new_start_date:
            if hasattr(new_start_date, "date"):
                new_start_date = new_start_date.date()
            if new_date < new_start_date:
                raise CustomAPIException(code=605, message="新销售日期早于当前对账单开始日期，禁止修改")

    # 2. 恢复旧库存（加回数量）
    goods = repo.goods.get_by_id(old_goods_id)
    restored_stock = float(goods["current_stock_num"]) + old_num
    ...
    if hasattr(data, "product_name") and data.product_name:
        # 先尝试恢复被软删除的商品
        restored_goods_id = repo.goods.restore_if_deleted(data.product_name, new_product_spec)
        if restored_goods_id:
            # 商品被恢复，重置库存和成本
            new_goods_id = restored_goods_id
            new_current_stock = 0
            new_unit_cost = 0.00
            # 重置商品库存和成本
            repo.goods.update_stock_and_cost(
            ...
            new_current_stock = float(new_goods["current_stock_num"])
    ...
    # 10. 触发成本重算
    from app.services.cost_recalc_service import recalculate_from_date
    # 确保old_sale_date是date对象
    if isinstance(old_sale_date, str):
        old_sale_date = datetime.strptime(old_sale_date, "%Y-%m-%d").date()
    # 如果商品变更了，需要重算旧商品和新商品
    recalc_date = old_sale_date if old_sale_date and old_sale_date < new_date else new_date
    if old_goods_id != new_goods_id:
        await recalculate_from_date(old_goods_id, recalc_date)
    await recalculate_from_date(new_goods_id, recalc_date)
...
async def get_sale_bill_detail(bill_id: int, end_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    for s in sale_list:
        ...
        if key not in merged_sales:
            ...
            sale_num = format_number(s["sale_num"])
            merged_sales[key] = {
...
async def export_sale_bill(bill_id: int, end_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "filename": f"销售对账单_{data['bill_info']['purchaser_name']}_{datetime.now().date().strftime('%Y%m%d')}.xlsx"
    }