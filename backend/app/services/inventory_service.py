...
async def list_inventory(
    ...
    for item in list_data:
        ...
        enriched_list.append({
            ...
            "inventory_num": format_number(item["current_stock_num"]),
...
async def get_inventory_detail(
    ...
    for flow in flow_list:
        ...
        formatted_flow.append({
            ...
            "change_num": format_number(flow["change_num"]),
            "change_date": flow["oper_time"].strftime("%Y-%m-%d %H:%M:%S"),
            "related_id": flow["biz_id"],
            "operator": None,  # 系统生成，无操作人
            "remark": flow["oper_source"],
            "stock_before": format_number(flow["stock_before"]),
            "stock_after": format_number(flow["stock_after"])
        })
    ...
    return {
        "inventory_info": {
            ...
            "inventory_num": format_number(goods["current_stock_num"]),
...
async def add_inventory_loss(data) -> Dict[str, Any]:
    ...
    # 日期格式校验
    try:
        loss_date = datetime.strptime(data.loss_date, "%Y-%m-%d").date()
    except ValueError:
    ...
    inventory_flow_repo.create({
        ...
        "stock_unit_cost": unit_cost,  # 记录当时的单位成本
        "oper_time": loss_date,
    ...
    await recalculate_from_date(goods["id"], loss_date)
...
async def list_inventory_loss(
    ...
    for item in list_data:
        formatted_list.append({
            ...
            "loss_num": format_number(item["loss_num"]),
...
async def delete_inventory_loss(id: int) -> None:
    ...
    loss_num = float(loss["loss_num"])
    loss_date = loss["loss_date"]  # 获取报损日期
    ...
    # 删除库存流动记录
    inventory_flow_repo.delete_by_biz(3, id)
    ...
    await recalculate_from_date(goods_id, loss_date)
