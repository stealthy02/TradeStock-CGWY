...
async def list_sale_bills(
    purchaser_name: Optional[str] = Query(None),
    receive_status: Optional[str] = Query(None, pattern="^(\\d+|)$"),
    ...
    """
    # 处理空字符串情况
    receive_status_int = int(receive_status) if receive_status and receive_status.strip() else None
    ...
    result = await sale_service.list_sale_bills(
        purchaser_name, receive_status_int, invoice_status_int, min_amount, max_amount, page_num, page_size
    )
    return ResponseModel(data=result)
