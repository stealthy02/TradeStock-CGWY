...
async def get_statistic_card():
    ...
    """
    result = await home_service.get_statistic_card_data()
    return ResponseModel(data=result)


@router.get("/home/pie-chart", response_model=ResponseModel[dict])
async def get_pie_chart(
    time_type: Optional[str] = Query("month", description="month/year/all"),
    ...
    Args:
        time_type (Optional[str]): 时间类型，可选值：month/year/all
        start_date (Optional[str]): 开始时间，格式：YYYY-MM-DD（已废弃）
        end_date (Optional[str]): 结束时间，格式：YYYY-MM-DD（已废弃）
    ...
    """
    # 参数逻辑校验 - 饼状图不再支持custom时间类型
    if time_type not in ["month", "year", "all"]:
        raise ParamErrorException(message="不支持的time_type参数，可选值：month/year/all")
