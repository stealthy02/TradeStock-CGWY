...
async def list_inventory(
    ...
    product_spec: Optional[str] = Query(None),
    ...
    result = await inventory_service.list_inventory(
        product_name, product_spec, min_num, max_num, sort_field, sort_order, page_num, page_size
    )
...
class InventoryLossAdd(BaseModel):
    ...
    @field_validator('product_spec', mode='before')
    @classmethod
    def convert_spec_to_str(cls, v):
        if v is not None:
            return str(v)
        return v
