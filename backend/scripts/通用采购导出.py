...
def merge_crease_line_data(list_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in list_data:
        ...
        purchase_date = _format_value(item.get("purchase_date"))
        
        # 判断是否是压痕线相关商品
        is_crease_line = "压痕线" in product_name
        
        if is_crease_line and purchase_date:
            # 压痕线且有日期，需要合并
            key = (purchase_date, "压痕线")
            
            if key in merged_dict:
                # 累加数值字段
                merged_item = merged_dict[key]
                merged_item["total_num"] = float(merged_item.get("total_num", 0) or 0) + float(item.get("total_num", 0) or 0)
                merged_item["total_kg"] = float(merged_item.get("total_kg", 0) or 0) + float(item.get("total_kg", 0) or 0)
                merged_item["total_price"] = float(merged_item.get("total_price", 0) or 0) + float(item.get("total_price", 0) or 0)
                # 单价取平均值（或保留第一个值，根据业务需求调整）
                if merged_item.get("unit_price") and item.get("unit_price"):
                    merged_item["unit_price"] = (float(merged_item["unit_price"]) + float(item["unit_price"])) / 2
                elif item.get("unit_price"):
                    merged_item["unit_price"] = item["unit_price"]
            else:
                # 新的合并项，深拷贝原始数据
            ...
            key = (purchase_date, product_name)
            merged_dict[key] = item.copy()
    
    # 转换回列表并保持原有顺序（按日期排序）
    merged_list = list(merged_dict.values())
    # 按采购日期排序，保持数据有序
    merged_list.sort(key=lambda x: _format_value(x.get("purchase_date")) or "")
...
def export(data: Dict[str, Any]) -> bytes:
    ...
    template_base_dir = base_dir / "public" / "purchase"
    
    entity_name = data["bill_info"].get("supplier_name", "")
    ...
    replace_mapping = {
        ...
        "{{bill_amount}}": _format_value(bill_info.get("bill_amount"), "numeric"),
    ...
    list_data = data.get("purchase_list", {}).get("list", [])
    # 合并压痕线数据
